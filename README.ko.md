...
| 집중 시작 | 기본 25분 타이머를 시작합니다. 딴짓 감지 중인 시간은 집중 시간에서 제외됩니다 |
| 집중 감지 | 현재 앱/사이트를 딴짓 목록에 추가하고, 브라우저 탭 감지를 켜거나 끕니다 |
| 크기 | 작게(96pt) / 보통(144pt) / 크게(192pt) |
| 옷 | 파란 체크 셔츠 / 주황 패딩 / I ♥ KIXLAB 티셔츠 / LG 트윈스 유니폼 |
| 말풍선 · 말풍선 빈도 | 끄기 / 드물게 · 보통 · 자주 |
| 언어 | 시스템 설정 따름 / 한국어 / English — 메뉴와 말풍선 문구가 함께 바뀝니다 |
| 소리 | 타자 소리 (기본 꺼짐) |
| 로그인 시 자동 실행 | |
| 입력 감지 권한 | 현재 상태 표시 + 시스템 설정 열기 |
| 위치 초기화 · 종료 | |

집중 중에는 앞에 있는 앱과(선택 시) 지원 브라우저의 현재 탭을 **기기 안에서만** 확인합니다.
딴짓 목록에 있는 앱·사이트를 6초 넘게 보고 있으면 캐릭터가 알려 줍니다. 키 입력 내용,
URL, 방문 기록은 저장하거나 전송하지 않습니다.
...
- `ko` / `en` : 언어 설정에 맞는 쪽이 자동으로 쓰입니다 (둘 다 적어 주세요)
...
```bash
swift build
swift run DeskPetTests                        # 단위 테스트 78개
./.build/debug/DeskPet --selftest ./selftest  # 실제 창을 띄우는 점검 42개 (+ 창 모습 PNG)
```
